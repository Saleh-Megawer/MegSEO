...
namespace MegSEO\Checks\TwitterCard\Scoring;
...
final readonly class TwitterCardScoreContributionBuilder
{
    private const MAX_SCORE = 100.0;
    private const PENALTY_MISSING = 20.0;
    private const PENALTY_EMPTY = 15.0;
    private const PENALTY_INVALID_TYPE = 10.0;
    ...
    public function build(array $issues, array $warnings, array $suggestions, array $metadata = []): ScoreSummary
    {
        ...
        return new ScoreSummary(value: round($score, 1), contributors: $contributors, metadata: ['max_score' => self::MAX_SCORE, 'total_deductions' => round($deductions, 1), 'rationale' => $rationale]);
    }
    ...
    {
        return match (true) {
            str_contains($warning->message, 'type') => self::PENALTY_INVALID_TYPE,
